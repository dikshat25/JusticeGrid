...
async def run_third_opinion_agent(state: dict) -> dict:
    agent_name = "Judge"
    ...
    prompt_template = prompt_loader.load("third_opinion")
    formatted_prompt = prompt_template.replace("{case_data}", str(case_data))
    formatted_prompt += f"\n\nComplete Courtroom Debate Transcript:\n{transcript}"
    ...
    return {
        "third_opinion": report.model_dump(),
        "timeline": timeline,
